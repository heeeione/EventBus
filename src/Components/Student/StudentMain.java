...
public class StudentMain {
	public static void main(String args[]) throws FileNotFoundException, IOException, NotBoundException {
		...
		while (!done) {
			...
			for (int i = 0; i < eventQueue.getSize(); i++) {
				...
				case Enrolment:
					printLogEvent("Get", event);
					eventBus.sendEvent(new Event(EventId.ListStudentsRegistered, getStudent(studentsList, event.getMessage())));
					break;
				case QuitTheSystem:
					printLogEvent("Get", event);
	...
	private static String getStudent(StudentComponent studentsList, String message) {
		String sID = message.split(" ")[0];
		Student student = studentsList.getStudent(sID);
	...
	private static String registerStudent(StudentComponent studentsList, String studentInfo) {
	...
	private static String deleteStudent(StudentComponent studentsList, String sID) {
		if (studentsList.deleteStudent(sID)) return "This student is successfully deleted.";
		else return "This student doesn't exist.";
	}
}